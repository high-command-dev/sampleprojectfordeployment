...
export default function VerifyRegisterOtp() {
  ...
  const onSubmit = async (values) => {
    try {
      const { data } = await authService.verifyRegisterOtp(values);
  ...
  return (
    <section className="mx-auto max-w-md glass-panel p-8">
      ...
      <p className="mt-2 text-sm text-slate-300">Enter the email used during registration and the 6-digit code sent to your inbox.</p>
